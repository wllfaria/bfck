...
/// parser / interpreter for brainfuck
pub struct Parser<W, R>
where
    W: Write,
    R: Read,
{
...
impl<W, R> std::fmt::Debug for Parser<W, R>
where
    W: Write,
    R: Read,
{
...
where
    ...
    R: Read,
{
    pub fn new(writer: W, reader: R) -> Parser<W, R> {
        Parser {
    ...
    pub fn interpret(&mut self, tokens: Vec<Token>) {
        while self.instruction_ptr < tokens.len() {
            match &tokens[self.instruction_ptr] {
                Token::MoveLeft(count) => self.move_ptr_left(*count),
                Token::MoveRight(count) => self.move_ptr_right(*count),
                Token::Increment(count) => self.increment_ptr(*count),
                Token::Decrement(count) => self.decrement_ptr(*count),
                Token::Write(count) => self.write_ptr(*count),
                Token::JumpIfZero(count) => self.jump_if_zero(*count, &tokens),
                Token::JumpUnlessZero(count) => self.jump_unless_zero(*count, &tokens),
                Token::Read(count) => self.read_ptr(*count),
            }
        }
    }
    ...
    fn move_ptr_left(&mut self, count: usize) {
    ...
    /// by increasing the tape by `INCREMENT` stops to simulate a "infinite" tape
    /// as the specification suggests.
    fn move_ptr_right(&mut self, count: usize) {
    ...
    fn increment_ptr(&mut self, count: usize) {
    ...
    fn decrement_ptr(&mut self, count: usize) {
    ...
    fn write_ptr(&mut self, count: usize) {
        for _ in 0..count {
            _ = write!(self.writer, "{}", self.tape[self.data_ptr] as char);
        }
        self.instruction_ptr += 1;
    }
    ...
    /// cell
    fn read_ptr(&mut self, count: usize) {
        println!("lol? {count}");
        for _ in 0..count {
            let mut byte = [0u8; 1];
            println!("{byte:?}");
    ...
    fn jump_if_zero(&mut self, _: usize, tokens: &[Token]) {
        ...
        let mut index = self.instruction_ptr + 1;
        while open_jumps != 0 {
            if index >= tokens.len() {
    ...
    /// next repeated occurrences directly to the next non-repeated token
    fn jump_unless_zero(&mut self, _: usize, tokens: &[Token]) {
        let curr_val = self.tape[self.data_ptr];
        if curr_val == 0 {
            self.instruction_ptr += 1;
            return;
...
mod tests {
    ...
    fn check<R>(input: &str, reader: R) -> Parser<Writer, R>
    where
        R: Read,
    {
        let writer = Writer::default();
        let mut parser = Parser::<Writer, R>::new(writer, reader);
    ...
    fn move_out_of_bounds_should_grow() {
        ...
        assert_eq!(parser.data_ptr, 12);
        assert_eq!(parser.tape.len(), 48);

        let source = (0..20).map(|_| ">").collect::<String>();
        let parser = check(&source, std::io::Cursor::new(""));
        assert_eq!(parser.data_ptr, 36);
        assert_eq!(parser.tape.len(), 48);
    }
    ...
    fn increment_and_decrement_wrapping_cells() {
        let source = "+++>--->---++++";
        let parser = check(source, std::io::Cursor::new(""));
        assert_eq!(parser.data_ptr, 18);
        assert_eq!(parser.tape[16], 3);
        assert_eq!(parser.tape[17], 253);
        assert_eq!(parser.tape[18], 1);
        assert_eq!(parser.tape.len(), 32);
    }
